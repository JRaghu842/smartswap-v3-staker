...
/// @title SmartSwap Staker Interface
/// @notice Allows staking nonfungible liquidity tokens in exchange for reward tokens
interface ISmartswapStaker is IERC721Receiver, IMulticall {
    /// @param rewardToken The token being distributed as a reward
    /// @param pool The SmartSwap pool
    ...
    /// @notice The SmartSwap Factory
    function factory() external view returns (ISmartSwapFactory);
    ...
    /// @notice Withdraws a SmartSwap LP token `tokenId` from this contract to the recipient `to`
    ...
    /// @notice Event emitted when a SmartSwap LP token has been staked
    /// @param tokenId The unique identifier of an SmartSwap LP token
    /// @param liquidity The amount of liquidity staked
    /// @param incentiveId The incentive in which the token is staking
    event TokenStaked(uint256 indexed tokenId, bytes32 indexed incentiveId, uint128 liquidity);

    /// @notice Event emitted when a SmartSwap LP token has been unstaked
    /// @param tokenId The unique identifier of an SmartSwap LP token
    /// @param incentiveId The incentive in which the token is staking
